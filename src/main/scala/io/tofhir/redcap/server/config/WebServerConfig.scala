package io.tofhir.redcap.server.config
...
/**
 * Keeps the configuration of web server
 *
...
class WebServerConfig(webserverConfig: Config) {
  ...
  lazy val serverPort: Int = Try(webserverConfig.getInt("port")).getOrElse(8095)
  ...
  lazy val baseUri: String = Try(webserverConfig.getString("base-uri")).getOrElse("tofhir-redcap")
}
